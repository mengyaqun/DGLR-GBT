...
%   options: Struct value in Matlab. The fields in options that can be set:可设置的字段
%                  
%           NeighborMode -  Indicates how to construct the graph. Choices
%                           are: [Default 'KNN']表示如何构建图形。[默认 'KNN］
%                'KNN'            -  k = 0
%                                       Complete graph完整图形
%                                    k > 0
...
%当且仅当两个节点互为 k 个近邻时，在两个节点之间添加一条边。您需要在选项中提供参数 k。默认 k=5。
%               'Supervised'      -  k = 0
%                                       Put an edge between two nodes if and
%                                       only if they belong to same class. 
% 在两个节点之间添加一条边，条件是只有当两个节点属于同一类别时，
%                                    k > 0
...
%                                       each other. 
%                                    Default: k=0
% 如果两个节点属于同一类别，并且它们是彼此的 k 个近邻，则在它们之间添加一条边。默认值：k=0
%                                   You are required to provide the label
%                                   information gnd in the options.
% 您需要在选项中提供标签信息 信息 gnd。                                             
